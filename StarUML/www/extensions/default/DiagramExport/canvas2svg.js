...
/**
 * fillText, strokeText is modified to support textAlign, textBaseLine
 *
...
;(function() {
    ...
    STYLES = {
        ...
        // modified by Minkyu Lee (2014. 07. 05)
        "lineDash":{
    ...
    /**
     * Set width of Canvas
     * added by Minkye Lee (2014. 07. 05)
     */
    ...
    /**
     * Set setLineDash
     * Add by Minkyu Lee (2014. 07. 05)
     */
    ctx.prototype.setLineDash = function (pattern){
        if (Array.isArray(pattern) && pattern.length > 0) {
            this.lineDash = pattern.join(",");
        }
    };
    ...
    ctx.prototype.__parseFont = function() {
        var font = this.font.trim(), parts, token, index = 0, data = {
            style : "normal",
            size : "10px",
            family : "sans-serif",
            weight: "normal",
    ...
    ctx.prototype.__applyText = function(text, x, y, action) {
        ...
        // Modified by Minkyu Lee
        if (this.textAlign) {
            this.__ctx.font = font.style + " " + font.size + " " + font.family;
            switch (this.textAlign) {
            case "left":
                // nothing to do
                break;
            case "right":
                x = x - this.__ctx.measureText(text).width;
                break;
            case "center":
                x = x - (this.__ctx.measureText(text).width / 2);
                break;
            }
        }
        if (this.textBaseline) {
            var fontSize = parseInt(font.size.replace(/px|pt|em/, ""));
